...
**Three user IDs tracked by the kernel:**

1. **Real UID (RUID):** The user who launched the process
2. **Effective UID (EUID):** Used for permission checks
3. **Saved UID (SUID):** Allows temporary privilege dropping

**For our exploit:**
```c
// When level08 user executes the binary:
RUID = 1008   // level08 user ID
EUID = 1009   // level09 user ID (from SUID bit)
SUID = 1009   // Saved for privilege restoration
```

**Permission check flow:**
```c
// Simplified kernel logic:
int check_file_permission(struct file *file, int uid) {
    if (file->owner == uid) {
        return ALLOW;
    }
    // ... group checks, other checks ...
    return DENY;
}

// When fopen is called:
check_file_permission(file, current->euid);  // Uses EUID, not RUID!
```

### Why Relative Paths are Dangerous in Privileged Programs

**Absolute path (safe):**
```c
strcpy(backup_path, "/var/backups/secure/");  // Always /var/backups/secure
```

**Relative path (vulnerable):**
```c
strcpy(backup_path, "./backups/");  // Depends on CWD!
```

**Attack surface:**
```
CWD=/home/users/level08  → ./backups/ = /home/users/level08/backups/  (intended)
CWD=/tmp                 → ./backups/ = /tmp/backups/                  (attacker controlled)
CWD=/dev/shm             → ./backups/ = /dev/shm/backups/              (attacker controlled)
CWD=/var/tmp             → ./backups/ = /var/tmp/backups/              (attacker controlled)
```
...
```c
const char *BACKUP_DIR = "/var/backups/secure/";
strcpy(backup_path, BACKUP_DIR);
```

2. **Validate and sanitize paths:**
```c
char *realpath_result = realpath(argv[1], NULL);
if (realpath_result == NULL || strstr(realpath_result, "..") != NULL) {
    fprintf(stderr, "Invalid path\n");
    exit(1);
}
```

3. **Drop privileges before file operations:**
```c
// Drop to real UID
setuid(getuid());
// Now EUID = RUID, no elevated privileges
fopen(user_controlled_path, "r");  // Safe
```

### BONUS: Format String Vulnerability in log_wrapper

The `log_wrapper` function contains a **secondary vulnerability** (CWE-134: Uncontrolled Format String):
...
void log_wrapper(FILE *log_file, char *prefix, char *msg)
{
    ...
    // VULNERABLE: msg is used as format string!
    snprintf(buffer + strlen(buffer), 254 - strlen(buffer), msg);
...
**The bug:** `msg` (which comes from `argv[1]`) is used directly as a format string in `snprintf`.
...
**Output:**
```
LOG: Starting back up: 0x7fffffffe490.0x7fffffffe498.0x7ffff7ffe710.0x7025702e70252e70.0x252e70252e70252e.0x2e70252e70252e70.0x70252e70252e7025.0xa
ERROR: Failed to open /home/users/level08/%p.%p.%p.%p.%p.%p.%p.%p
```

**What happened:**
- Format specifiers `%p` leaked **stack addresses**
- Fourth leak: `0x7025702e70252e70` = ".p.%p.%p" in hex (our own input!)
- Could potentially leak sensitive data or corrupt memory

**Why this is interesting but not critical here:**
- The log file is written to `./backups/.log` (we can already read it)
- No easy way to leverage this for code execution in this context
- Demonstrates that **multiple vulnerabilities** can exist in the same program
- In real-world scenarios, chaining vulnerabilities is powerful!

**Secure fix:**
```c
// Use a format string literal:
...
**Historical examples of SUID exploits:**

1. **CVE-2021-3560 (Polkit):** SUID binary with race condition → local root
2. **CVE-2016-5195 (Dirty COW):** SUID + race condition → arbitrary file write
3. **CVE-2019-14287 (sudo):** SUID + integer overflow → bypass authentication

**Why SUID binaries are risky:**
- Single bug = privilege escalation
- Attack surface available to unprivileged users
- Often written in C (memory safety issues)
- Complex to audit thoroughly

**Best practices:**
- **Minimize SUID binaries** (use capabilities on Linux instead)
- **Always use absolute paths** in privileged code
- **Drop privileges ASAP** after necessary operations
- **Validate all user input** (paths, arguments, environment)
- **Audit regularly** with tools like `checksec`, `patchelf`
